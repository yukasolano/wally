...
public class FileExporterMovimentos extends AbstractFileExporter {
	...
	private static String[] columnsDividendo = { "Código", "Data", "Quantidade", "Valor unitário" };
	...
	public ExportedFile export() {
		...
		// Create a Sheet
		Sheet sheetDividendo = workbook.createSheet("Dividendo");
		// Create a Row
		Row headerRowDividendo = sheetDividendo.createRow(0);

		// Create cells
		for (int i = 0; i < columnsDividendo.length; i++) {
			Cell cell = headerRowDividendo.createCell(i);
			cell.setCellValue(columnsDividendo[i]);
			cell.setCellStyle(headerCellStyle);
		}
		...
		Iterable<DividendoEntity> dividendos = dividendoRepository.findAll();
		for (DividendoEntity dividendo : dividendos) {
			Row row = sheetDividendo.createRow(rowNum++);

			row.createCell(0).setCellValue(dividendo.getCodigo());

			Cell data = row.createCell(1);
			data.setCellValue(convertToDate(dividendo.getData()));
			data.setCellStyle(dateCellStyle);

			row.createCell(2).setCellValue(dividendo.getQuantidade());
			row.createCell(3).setCellValue(dividendo.getValorUnitario());

		}

		// Resize all columns to fit the content size
		for (int i = 0; i < columnsDividendo.length; i++) {
			sheetDividendo.autoSizeColumn(i);
		}
